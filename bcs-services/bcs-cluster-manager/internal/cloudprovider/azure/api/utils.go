...
// setTags 设置tags
func (c *nodeGroupToPool) setKubelet() {
	kubelet := &armcontainerservice.KubeletConfig{}
	if c.group.NodeTemplate.ExtraArgs[common.Kubelet] != "" {
		json.Unmarshal([]byte(c.group.NodeTemplate.ExtraArgs[common.Kubelet]), kubelet)
	}
	c.pool.Properties.KubeletConfig = kubelet
}
