...
import (
	...
	"github.com/Tencent/bk-bcs/bcs-services/bcs-cluster-manager/internal/cloudprovider/gke/api"
...
func DeleteGKEClusterTask(taskID string, stepName string) error {
	...
	if err != nil {
		blog.Errorf("DeleteGKEClusterTask[%s]: get azure client for cluster[%s] in task %s step %s failed, %s",
			taskID, clusterID, taskID, stepName, err.Error())
	...
	if cluster.ClusterName != "" {
		err = cli.DeleteCluster(context.Background(), cluster.ClusterName)
		if err != nil {
			blog.Errorf("DeleteGKEClusterTask[%s]: task[%s] step[%s] call azure DeleteGKECluster failed: %v",
