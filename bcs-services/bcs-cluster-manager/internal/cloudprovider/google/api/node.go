...
func (n *NodeManager) transInstanceIDsToNodes(ids []string, opt *cloudprovider.ListNodesOption) ([]*proto.Node,
	...
	insList, err := client.ListZoneInstanceWithFilter(context.Background(), opt.Common.Region, InstanceNameFilter(ids))
	if err != nil {
...
func InstanceToNode(cli *ComputeServiceClient, ins *compute.Instance) *proto.Node {
	zoneInfo, _ := GetResourceInfo(ins.Zone)
	...
	machineInfo, _ := GetResourceInfo(ins.MachineType)
