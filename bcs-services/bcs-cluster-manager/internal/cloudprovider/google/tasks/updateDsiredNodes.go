...
func ApplyInstanceMachinesTask(taskID string, stepName string) error {
	...
	_, err = applyInstanceMachines(ctx, dependInfo, uint64(nodeNum))
	if err != nil {
...
func applyInstanceMachines(ctx context.Context, info *cloudprovider.CloudDependBasicInfo, nodeNum uint64) (
	*compute.Operation,
	error) {
	...
	err = cloudprovider.LoopDoFunc(context.Background(), func() error {
		o, err := api.GetOperation(client, operation.SelfLink)
		if err != nil {
			return err
		}
		if o.Status == "DONE" {
		...
		blog.Infof("taskID[%s] applyInstanceMachines still running", taskID)
		return nil
	...
	err = cloudprovider.LoopDoFunc(ctx, func() error {
		instances, err := api.ListInstanceGroupsInstances(client, info.NodeGroup.AutoScaling.AutoScalingID)
		if err != nil {
			blog.Errorf("applyInstanceMachines[%s] ScaleOutInstances failed: %v", taskID, err)
			return err
		}
		for _, inst := range instances {
			if inst.Status != api.InstanceStatusRunning {
				blog.Infof("taskID[%s] applyInstanceMachines is still running, instance: %s, status: %s", taskID,
	...
	if err != nil {
		blog.Errorf("taskID[%s] applyInstanceMachines failed: %v", taskID, err)
		return nil, err
	}
...
func checkClusterInstanceStatus(ctx context.Context, info *cloudprovider.CloudDependBasicInfo,
	...
	err = cloudprovider.LoopDoFunc(timeCtx, func() error {
		instances, err := cli.ListZoneInstanceWithFilter(ctx, info.NodeGroup.Region, api.InstanceNameFilter(instanceIDs))
		if err != nil {
