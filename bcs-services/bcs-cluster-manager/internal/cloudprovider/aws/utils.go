...
var (
	...
	checkEKSNodeGroupsStatusStep = cloudprovider.StepInfo{
		StepMethod: fmt.Sprintf("%s-CheckEKSNodeGroupsStatusTask", cloudName),
		StepName:   "检测集群节点池状态",
	}
	updateEKSNodeGroupsToDBStep = cloudprovider.StepInfo{
		StepMethod: fmt.Sprintf("%s-UpdateEKSNodesGroupToDBTask", cloudName),
		StepName:   "更新节点池信息",
	}
...
func (cn *CreateClusterTaskOption) BuildUpdateNodeGroupsToDBStep(task *proto.Task) {
	updateStep := cloudprovider.InitTaskStep(updateEKSNodeGroupsToDBStep)
	...
	task.Steps[updateEKSNodeGroupsToDBStep.StepMethod] = updateStep
	task.StepSequence = append(task.StepSequence, updateEKSNodeGroupsToDBStep.StepMethod)
}
