...
func CheckCloudNodeGroupStatusTask(taskID string, stepName string) error {
	...
	client, err := api.NewAWSClientSet(cmOption)
	if err != nil {
		blog.Errorf("CheckCloudNodeGroupStatusTask[%s]: get aws clientSet for nodegroup[%s] failed, %s",
	...
	err = loop.LoopDoFunc(ctx, func() error {
		ng, errPool := client.DescribeNodegroup(&group.CloudNodeGroupID, &cluster.SystemID)
		if errPool != nil {
			blog.Errorf("CheckCloudNodeGroupStatusTask[%s] DescribeNodegroup[%s] failed: %v", taskID,
		...
		case *ng.Status == api.NodeGroupStatusActive:
			return loop.EndLoop
		case *ng.Status == api.NodeGroupStatusCreateFailed:
			return fmt.Errorf("create nodegroup[%s] failed, status[%s]", group.CloudNodeGroupID, *ng.Status)
		default:
			return nil
		}
	...
	err = cloudprovider.GetStorageModel().UpdateNodeGroup(context.Background(), generateNodeGroupFromAsgAndLtv(group,
		asgInfo[0], ltvInfo[0]))
	if err != nil {
		blog.Errorf("CheckCloudNodeGroupStatusTask[%s]: UpdateNodeGroup[%s] failed, %s",
			taskID, nodeGroupID, err.Error())
		retErr := fmt.Errorf("CheckCloudNodeGroupStatusTask UpdateNodeGroup[%s] err, %s",
