...
func WithConnection(ctx context.Context, conn *sql.Conn, config *Config,
	...
	if config.DatabaseName == "" {
		query := `SELECT SF_GET_SCHEMA_NAME_BY_ID(CURRENT_SCHID)`
...
func (m *Mysql) ensureVersionTable() (err error) {
	...
	var result string
	query := `SELECT TABLE_NAME FROM USER_TABLES WHERE TABLE_NAME = '` + m.config.MigrationsTable + `'`
	...
	query = "CREATE TABLE " + m.config.MigrationsTable +
		"(version BIGINT NOT NULL PRIMARY KEY, dirty BIT NOT NULL)"
	if _, err := m.conn.ExecContext(context.Background(), query); err != nil {
		return &database.Error{OrigErr: err, Query: []byte(query)}
	}
	return nil
