...
func (s mysql) HasForeignKey(tableName string, foreignKeyName string) bool {
	...
	s.db.QueryRow(fmt.Sprintf("SELECT count(*) FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE CONSTRAINT_SCHEMA=\"%s\" AND TABLE_NAME=\"%s\" AND CONSTRAINT_NAME=\"%s\" AND CONSTRAINT_TYPE='FOREIGN KEY'", currentDatabase, tableName, foreignKeyName)).Scan(&count)
	return count > 0
}

func (s mysql) HasTable(tableName string) bool {
	currentDatabase, _ := currentDatabaseAndTable(&s, tableName)
	var count int
	err := s.db.QueryRow(fmt.Sprintf("SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = \"%s\" AND TABLE_NAME = \"%s\"", currentDatabase, tableName)).Scan(&count)
	if err != nil {
...
func (s mysql) HasIndex(tableName string, indexName string) bool {
	currentDatabase, _ := currentDatabaseAndTable(&s, tableName)
	var exists int
	err := s.db.QueryRow(fmt.Sprintf("SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = \"%s\" AND TABLE_NAME = \"%s\" AND INDEX_NAME = \"%s\"", currentDatabase, tableName, indexName)).Scan(&exists)
	if err != nil {
		if err == sql.ErrNoRows {
			return false
		}
...
func (s mysql) HasColumn(tableName string, columnName string) bool {
	currentDatabase, _ := currentDatabaseAndTable(&s, tableName)
	var exists int
	err := s.db.QueryRow(fmt.Sprintf("SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = \"%s\" AND TABLE_NAME = \"%s\" AND COLUMN_NAME = \"%s\"", currentDatabase, tableName, columnName)).Scan(&exists)
	if err != nil {
		if err == sql.ErrNoRows {
			return false
		}
